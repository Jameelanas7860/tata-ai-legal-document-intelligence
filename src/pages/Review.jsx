...
export default function Review() {
  ...
  const [data, setData] = useState({ risks: [], summary: { overall: 'Low', high: 0, medium: 0, low: 0, confidence: 0 }, contractSummary: '', document: null });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await riskService.getRiskAnalysis();
        if (active) {
          setData(result);
          setError(null);
        }
  ...
  const { risks, riskSummary, contractSummary, document: docInfo } = {
    risks: data.risks,
  ...
  const handleAction = useCallback(async (risk, key) => {
    if (key === 'escalate') {
      setEscalateModal(risk);
      setEscalateNote('');
      return;
    }
    setActionLog((log) => [...log, { risk: risk.title, action: key, id: Date.now() }]);
    try {
      await riskService.updateRiskStatus(risk.documentId, risk.dbId, key, '');
    } catch (err) {
  ...
  const confirmEscalation = useCallback(async () => {
    ...
    setActionLog((log) => [...log, { risk: escalateModal.title, action: 'escalate', id: Date.now() }]);
    try {
      await riskService.updateRiskStatus(escalateModal.documentId, escalateModal.dbId, 'escalate', note);
    } catch (err) {
      console.error('Failed to escalate risk:', err);
    }
    setEscalateModal(null);
    setEscalateNote('');
  }, [escalateModal, escalateNote]);
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sort === 'Page') return (a.page || 0) - (b.page || 0);
      return 0;
    });
    return list;
  }, [filter, sort, risks]);
  ...
  return (
    <div>
      ...
      <div className="card mb-6 p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-900">{docInfo?.name || 'Document'}</h2>
              <p className="text-sm text-slate-500">
                {String(docInfo?.id || '').slice(0, 8)} · {docInfo?.jurisdiction || 'N/A'}
              </p>
            </div>
          </div>
          <StatusBadge status={docInfo?.status || 'Processing'} />
        </div>
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <MetaItem icon={Tag} label="Document Type" value={docInfo?.type || 'N/A'} />
          <MetaItem icon={MapPin} label="Jurisdiction" value={docInfo?.jurisdiction || 'N/A'} />
          <MetaItem icon={Eye} label="Review Status" value={docInfo?.status || 'Processing'} />
        </div>
      </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => { setEscalateModal(null); setEscalateNote(''); }}>Cancel</Button>
            <Button variant="danger" onClick={confirmEscalation}>Confirm Escalation</Button>
          </>
        }
      >
        {escalateModal ? (
